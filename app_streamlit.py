...
# --------------------------------------------------------
# Colonnes officielles Backstage
# --------------------------------------------------------
COLS = {
...
def creator_type(row, hist, cfg):
    ...
    if hist is not None and not hist.empty:
        ...
        if not h.empty and h["diamants"].max() >= thr:
            ever = True
    if not ever and "confirmé" in str(row.get("statut_diplome","")).lower():
        ever = True
...
# --------------------------------------------------------
# Calculs Agents & Managers
# --------------------------------------------------------
def totals_active_by(group_field, creators_df):
...
def cumulative_two_months(curr, prev, key):
    curr = curr.copy()
    curr.rename(columns={"diamants_actifs":"diamants_mois"}, inplace=True)
    if prev is not None and not prev.empty:
        prev = prev[[key, "diamants_actifs"]].copy()
        prev.rename(columns={"diamants_actifs":"diamants_mois_prec"}, inplace=True)
        out = pd.merge(curr, prev, on=key, how="left").fillna({"diamants_mois_prec":0})
    else:
        out = curr
        out["diamants_mois_prec"] = 0.0
    out["diamants_cumules"] = out["diamants_mois"] + out["diamants_mois_prec"]
    return out
...
def agent_manager_bonus(crea_df, group_field, b2_amt, b3_amt):
    tmp = crea_df.copy()
...
st.caption("Trois onglets • Base unique créateurs • Exports CSV & PDF")

col1, col2, col3 = st.columns(3)
with col1:
    f_current = st.file_uploader("Mois courant", type=["xlsx","xls","csv"], key="cur")
with col2:
    f_previous = st.file_uploader("Mois précédent (pour cumuls)", type=["xlsx","xls","csv"], key="prev")
with col3:
    f_yaml = st.file_uploader("Seuils d’activité (YAML)", type=["yml","yaml"], key="yaml")
